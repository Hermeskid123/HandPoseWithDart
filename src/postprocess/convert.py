...
UNITY_BASIS = torch.tensor(
    [
        [0.98252, 0.1861579, 8.797169e-08, -1.925538e-07],
        [0.9867496, 0.1622507, 8.623259e-07, -5.569176e-06],
        [0.998917, 0.04652741, -3.814051e-07, -8.645397e-07],
        [0.9845469, 0.1751214, -6.773092e-08, -1.296573e-07],
        [0.9895474, 0.1442084, 2.347626e-07, 4.719131e-08],
        [0.9987367, 0.0502522, 3.299475e-07, 2.839107e-07],
        [0.9802657, 0.1976843, -4.922589e-09, -2.051222e-07],
        [0.9878246, 0.1555727, -1.964481e-07, 5.758296e-08],
        [0.9954529, 0.09525843, 4.293424e-07, 1.511594e-06],
        [0.9996653, 0.02587354, -1.435634e-07, 1.650499e-07],
        [0.9994696, 0.03256664, 2.490357e-07, -3.985346e-08],
        [0.9999212, 0.01256056, -1.017585e-07, -2.349624e-09],
        [-0.9291456, -0.3697141, 2.073295e-07, -2.429503e-07],
        [-0.9564937, -0.2917534, 3.610364e-06, -1.518497e-05],
        [-0.9723085, -0.233702, 2.076902e-08, -6.052665e-07],
    ],
    dtype=torch.float,
    device=device,
)
MANO_BASIS = torch.tensor(
    [
        [0.9973, -0.0215, -0.0324, 0.0616],
...
def get_root_aa(root_rot):
    ...
    mano_to_unity_quat = torch.tensor(
        [0.9928, 0.0000, 0.1160, -0.0300], dtype=torch.float, device=device
    )
    ...
    with xlrd.open_workbook(excel_path) as wb:
        ...
        for row in sheet.get_rows():
            if 1 <= idx <= 26:
                unity_rot.append(
                    torch.tensor(
                        [float(x) for x in row[rotation_data_index].value.split(",")],
                        dtype=torch.float,
                        device=device,
                    )
                )
...
if __name__ == "__main__":
    base_path = r"/home/preston/Public/3degree/"
    for root, dirs, _ in os.walk(base_path):
        for d in dirs:
            print(os.path.join(root, d))
            excel_path = os.path.join(root, d)
