...
class DARTset:
    def __init__(
        self, data_split="train", use_full_wrist=True, load_wo_background=False
    ):
        self.name = "DARTset"
    ...
    def load_dataset(self):
        ...
        image_parts = [
            ...
            if os.path.isdir(os.path.join(self.root, r))
            and "verts" not in r
            and "wbg" not in r
        ]
        ...
        for imgs_dir in image_parts:
            ...
            data_record = pickle.load(
                open(os.path.join(self.root, f"part_{imgs_dir}.pkl"), "rb")
            )
            for k in range(len(data_record["pose"])):
                ...
                verts_3d_path = os.path.join(
                    imgs_path + "_verts", data_record["img"][k].replace(".png", ".pkl")
                )
    ...
    def get_joints_3d(self, idx):
        ...
        joints = (
            joints - joints[9] + np.array([0, 0, 0.5])
        )  # * We use ortho projection, so we need to shift the center of the hand to the origin
        return joints
    ...
    def get_image(self, idx):
        ...
        if self.load_wo_background:
            ...
            path = os.path.join(
                *path.split("/")[:-2], path.split("/")[-2] + "_wbg", path.split("/")[-1]
            )
    ...
    def get_mano_pose(self, idx):
        ...
        new_pose = np.concatenate(
            [root.reshape(-1), pose[1:].reshape(-1) + self.MANO_pose_mean], axis=0
        )  # [48]
