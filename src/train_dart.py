...
from DartSimple import DartSimple, DartDataset
...
def train(model, train_loader, val_loader, epochs=10, lr=1e-4, device="cuda"):
    ...
    for epoch in range(epochs):
        ...
        for images, keypoints_gt in tqdm(train_loader, desc="Training", leave=False):
            ...
            # print("gt ", keypoints_gt.shape)
            optimizer.zero_grad()
            keypoints_pred = model(images)
            # print("pred ", keypoints_pred.shape)
        ...
        print(f"Train Loss: {epoch_loss:.3f} mm | Val Loss: {val_loss:.3f} mm")

        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), "dart_model_best.pth")
...
if __name__ == "__main__":
    image_dir = "data/DARTset/train/0_wbg/"
    label_file = "data/DARTset/train/part_0.pkl"
    ...
    full_dataset = DartDataset(image_dir, label_file, transform=transform)
